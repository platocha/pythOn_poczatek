    ...
    def reserve_product(product, quantity):
        for available_product in Store.AVAILABLE_PRODUCTS:
            if available_product.product == product:
                if quantity > available_product.quantity:
                    raise TemporaryOutOfStock(product_name=product, available_quantity=available_product.quantity)
                available_product.quantity -= quantity
                return
        raise ProductNotAvailable(product_name=product)
